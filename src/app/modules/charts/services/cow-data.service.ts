...
export class CowDataService {
  ...
  getDataWithTimestamp(restUrl: string, timestamp: Date): Observable<any> {
    ...
    return this.http.get(restUrl, { params });
  }

  getSectionsDataAlgoThree(timestamp: Date, idCowShed: number) {
    // const dataUrl = "http://10.0.0.11:8080/timeLocation/getThirdAlgorithm/" + barnId;
    const path = 'thirdAlgorithm';
    const urlParams = {idCowShed};
    const url = this.ulrService.getUrl(path, urlParams);

    return this.getDataWithTimestamp(url, timestamp);
  }

  parseSectionPositionsToIndices(sections: Array<CowshedSection>): Array<CowshedSection> {
    let xPositionsMap = {};
    let yPositionsMap = {};
